...
import { TrendingUp, DollarSign, Bitcoin, ArrowDownToLine, ArrowUpFromLine, ArrowRight, BarChart3, PlusCircle, Activity, ShieldCheck, ShieldAlert, Clock, ChevronRight, Lightbulb, Sparkles, Repeat, Crown, Users, Copy, Check } from "lucide-react";
...
export function DashboardContent({ user, btcPriceData, recentTransactions }: DashboardContentProps) {
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-sm text-text-muted">Good {getGreeting()},</span>
            <div className="flex items-center gap-1.5 rounded-full bg-success/10 border border-success/20 px-2 py-0.5">
              <div className="h-1.5 w-1.5 rounded-full bg-success animate-pulse" />
              <span className="text-[10px] font-medium text-success">Online</span>
            </div>
          </div>
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-text-primary font-heading tracking-tight">
            {user.fullName}
          </h1>
        </div>
        <Link
          href="/dashboard/deposit"
          className="hidden sm:inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary-hover transition-all shadow-lg shadow-primary/20 hover:shadow-primary/30"
        >
    ...
    <motion.div variants={itemVariants}>
      <div className="grid grid-cols-2 gap-3">
        ...
        <Surface delay={0.1} variant="fiat" className="p-4">
          ...
          <p className="text-lg lg:text-xl font-bold text-text-primary tracking-tight font-heading financial-number">
            {formatCurrency(user.fiatBalance, user.currency)}
          </p>
          <p className="text-[11px] font-medium text-text-muted mt-0.5">Live Balance</p>
          <div className="mt-2 h-[3px] rounded-full bg-primary/20 overflow-hidden">
            <div className="h-full bg-primary rounded-full" style={{ width: totalPortfolio > 0 ? `${Math.min((user.fiatBalance / totalPortfolio) * 100, 100)}%` : '0%' }} />
          </div>
        </Surface>

        {/* Profit */}
        <Surface delay={0.15} variant="profit" className="p-4 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <p className="text-lg lg:text-xl font-bold text-success tracking-tight font-heading financial-number glow-success">
              {formatCurrency(user.profitBalance, user.currency)}
            </p>
            <p className="text-[11px] font-medium text-text-muted mt-0.5">Profit</p>
            <div className="mt-2 h-[3px] rounded-full bg-success/20 overflow-hidden">
              <div className="h-full bg-success rounded-full" style={{ width: totalPortfolio > 0 ? `${Math.min((user.profitBalance / totalPortfolio) * 100, 100)}%` : '0%' }} />
            </div>
          </div>
        </Surface>

        {/* Bitcoin */}
        <Surface delay={0.2} variant="btc" className="p-4 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="flex items-center gap-1 mt-0.5">
              <p className="text-[11px] font-medium text-text-muted">Bitcoin</p>
              <span className={cn(
                "text-[10px] font-medium",
                isBtcPositive ? "text-success" : "text-destructive"
              )}>
                {isBtcPositive ? "+" : ""}{btcChange24h.toFixed(1)}%
              </span>
            </div>
            <div className="mt-2 h-[3px] rounded-full bg-warning/20 overflow-hidden">
              <div className="h-full bg-warning rounded-full" style={{ width: totalPortfolio > 0 ? `${Math.min((btcValueInFiat / totalPortfolio) * 100, 100)}%` : '0%' }} />
            </div>
          </div>
        </Surface>

        {/* Active Investments */}
        <Surface delay={0.25} className="p-4">
          <div className="flex items-center gap-2.5 mb-3">
          ...
          <p className="text-[11px] font-medium text-text-muted mt-0.5">Invested</p>
          <div className="mt-2 h-[3px] rounded-full bg-info/20 overflow-hidden">
            <div className="h-full bg-info rounded-full" style={{ width: totalPortfolio > 0 ? `${Math.min((user.activeInvestment / totalPortfolio) * 100, 100)}%` : '0%' }} />
          </div>
        </Surface>
      </div>
    </motion.div>
  ...
  return (
    ...
    >
      {/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
          MOBILE LAYOUT (< lg)
          Order: Welcome → Chart → Stats → Actions → Account → Activity
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
      <div className="lg:hidden space-y-5">
        {welcomeBanner}

        {/* BTC Chart — prominent, like the inspiration */}
        <motion.div variants={itemVariants}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-semibold text-text-muted uppercase tracking-wider">Market Overview</h2>
            <div className="flex items-center gap-1.5 text-xs text-text-muted">
              <div className="h-1.5 w-1.5 rounded-full bg-success animate-pulse" />
              <span>Live</span>
            </div>
          </div>
          <TradingViewWidget symbol="BITSTAMP:BTCUSD" height={240} />
        </motion.div>

        {/* 2×2 Stat Cards — immediately after chart */}
        {statCards}
        ...
        <motion.div variants={itemVariants}>
          <Surface className="p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-medium text-text-muted uppercase tracking-wider">Total Portfolio</p>
              <span className="text-lg font-bold text-text-primary font-heading financial-number">
      ...
      {/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
          DESKTOP LAYOUT (lg+)
          Two-column: Left 8/12 (hero + charts + activity) | Right 4/12 (stats + actions + account)
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
      <div className="hidden lg:block">
        <div className="mb-6">{welcomeBanner}</div>
          ...
          <div className="col-span-8 space-y-6">
            ...
            <motion.div variants={itemVariants}>
              <Surface className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <p className="text-xs font-medium text-text-muted uppercase tracking-wider">Total Portfolio Value</p>
                    <div className="flex items-baseline gap-3 mt-1">
                      <span className="text-4xl font-bold text-text-primary tracking-tight font-heading financial-number">
                        {formatCurrency(totalPortfolio, user.currency)}
                      </span>
            ...
            <motion.div variants={itemVariants}>
              ...
              <div className="grid gap-4 grid-cols-1 xl:grid-cols-2">
                <TradingViewWidget symbol="BITSTAMP:BTCUSD" height={280} />
...
// ─── RECENT ACTIVITY LIST (shared between mobile + desktop) ───
    ...
    return (
      <Surface delay={0.5} className="p-5">
        <div className="flex flex-col items-center justify-center py-4 text-center">
          ...
          <p className="mt-3 text-sm font-medium text-text-primary">Start your investment journey</p>
        ...
        {transactions.map((tx, index) => {
          ...
          return (
            <motion.div
              ...
              className="flex items-center justify-between p-2.5 rounded-lg hover:bg-surface-muted/30 transition-colors"
            >
              <div className="flex items-center gap-3">
                <div className={cn(
